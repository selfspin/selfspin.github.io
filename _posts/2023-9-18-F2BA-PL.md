...
$$
\min_{x,y \in Y^*(x)} f(x,y) \quad{\rm s.t.} \quad Y^*(x) = \arg \min_{y \in \mathcal{Y}} g(x,y).
$$
...
$$
\begin{align*}
\min_x \varphi(x), \quad {\rm where} \quad \varphi(x) = \min_{y \in Y^*(x)} f(x,y)
\end{align*}
$$
...
$$
\begin{align*}
\min_{x,y \in \mathcal{Y}} \sigma f(x,y) + g(x,y) - g^*(x), \quad {\rm where} \quad g^*(x) = \min_{y \in \mathcal{Y}} g(x,y).
\end{align*}
$$
...
$$
\begin{align*}
\varphi_{\sigma} := \frac{l(x,\sigma) - l(x,0)}{\sigma}  = \min_y \left\{ f(x,y) + \frac{g(x,y) - g^*(x)}{\sigma} \right\}
\end{align*}
$$
...
$$
\begin{align*}
\rho^{-1} \Vert y - {\rm prox}_{\rho h_{\sigma}(x,\,\cdot\,) } (y) \Vert \ge \mu ~{\rm dist}(y,Y_{\sigma}^*(x)),\quad {\rm where} \quad Y_{\sigma}^*(x) = \arg \min_{y \in \mathcal{Y}} h_{\sigma}(x,y) 
\end{align*}
$$
...
在上述条件下，可以证明 $Y_{\sigma}^*(x)$ 关于 $x$ 和 $\sigma$ 都具有Lipschitz性质，以下证明与 [5] 的结论类似。



对于任意的 $y_1 \in Y_{\sigma_1}^*(x_1)$, 都存在距离最近的 $y_2 \in Y_{\sigma_2}^*(x_2)$, 使得
...
$$
\begin{align*}
{\rm dist}(Y_{\sigma_1}^*(x_1), Y_{\sigma_2}^*(x_2)) \le  \mathcal{O}(L_g) \Vert x_1 - x_2 \Vert + \mathcal{O}(L_f) \Vert \sigma_1 - \sigma_2 \Vert.
\end{align*}
$$



上述性质的直接推论是 ${\rm dist}( Y_{\sigma}^*(x), Y^*(x)) \le \mathcal{O}(\sigma L_f / \mu) $.
...
对于任意的 $y_{\sigma}^*(x) \in Y_{\sigma}^*(x)$, 都存在距离最近的 $y^*(x) \in Y^*(x)$, 使得
...
&\le f(x,y^*(x)) - f(x,y_{\sigma}^*(x)) + \sigma^{-1}  (g(x,y_{\sigma}^*(x)) - g(x,y^*(x)) ) \\
&\le  C_f \Vert y_{\sigma}^*(x) - y^*(x) \Vert  + \frac{L_g}{2\sigma} \Vert y_{\sigma}^*(x) - y^*(x) \Vert^2 \\
...
遗憾的是，考虑 [5] 中给出的例子，上述关系式in general并不满足，因此我们需要考虑引入额外的条件。



首先，假设约束集合 $\mathcal{Y}$ 是一个满足以下的代数性质的凸集
...
为了保证约束问题集合的积极集对于扰动并不敏感，我们引入约束优化中常见的正则条件： 假设至少存在一个最优点 $y_{\sigma}^*(x) \in Y_{\sigma}^*(x)$，使得其对应的积极集中的所有 $\nabla_y h_i(y)$ 满足线性无关 (也称为LICQ正则条件)，并且存在一个对应的Lagrange乘子使得互补松弛条件满足。



## Proof: Part I

下面的记号采用 $w = (x,\sigma)$， $\theta = y$ , $p(w,\theta) = h_{\sigma}(x,y)$ 以及 $p^*(w) = l(x,\sigma)$. 假设代数性质 $\Theta = \{ h_i(\theta) \le 0\}$.
...
沿用上一个part的记号，我们知道 $v(w) = \min_{\theta \in \Theta} p(w,\theta)$. 我们考虑推广Danskin's 定理当 $\Theta^*(w)$ 满足 Lipschitz的情况.下面的证明可以在 [3] 中找到。
...
根据集合的Lipschitz性质，对于任意的 $\theta \in \Theta^*(w)$ 以及 ${\rm d} w $, 存在 $ \theta +t {\rm d} \theta \in \Theta^*(w + t{\rm d} w)$ 满足 $\Vert {\rm d} \theta \Vert \asymp  \Vert {\rm d} w \Vert $. 

根据一阶最优性条件，存在 $ \partial \mathcal{I}_{\Theta}(\theta)$ 使得 $ \nabla_{\theta} p(w,\theta) + \partial \mathcal{I}_{\Theta}(\theta) = 0$.  那么
...
$$
\begin{align*}
\nabla v(w) = p(w, \theta), \quad \forall \theta \in \Theta^*(w).
\end{align*}
$$
...
对于任意的扰动 ${\rm d} w$ , 根据集合的Lipschitz性质，对于任意的 $\theta \in \Theta^*(w)$ 以及 ${\rm d} w $, 存在 $ \theta + t{\rm d} \theta \in \Theta^*(w + t{\rm d} w)$ 满足 $\Vert {\rm d} \theta \Vert \asymp  \Vert {\rm d} w \Vert $.  
...
0 \\
\nabla_y f(x,y^*(x))
\end{bmatrix}, \quad \forall y^*(x) \in Y^*(x).
\end{align*}
$$
...
&= - [U^\top \nabla_{zz}^2 \mathcal{L}(w,z) U]^{-1} U^\top \mathcal{L}_{zw}^2 \mathcal{L}(w,z) {\rm d}w + o(1)
\end{align*}
$$
...
&= t\nabla_{ww}^2 p(w,\theta) {\rm d} w + 
tP^\top {\rm d}r + o(t) \\
&= t\nabla_{ww}^2 p(w,\theta) {\rm d} w- t P^\top [U^\top \nabla_{zz}^2 \mathcal{L}(w,z) U]^{-1} U^\top \mathcal{L}_{zw}^2 \mathcal{L}(w,z) {\rm d}w + o(t) \\
