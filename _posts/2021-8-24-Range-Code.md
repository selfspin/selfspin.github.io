...
再者，将$T$出发的Filter操作拆分为从$T_1,T_2$两部分出发的Filter。从$T_1$中的结点$x$出发的Filter操作，由于$x$仅有一个邻居$u$,仅有$T$中以$u$为邻居的结点才会经过Filter后被保留唯一元素$u$, 否则被Filter后应为空集。而$u$正好为$V$中的结点，而$V$中每个结点恰好有$d$个邻居，因此从$u$出发的Filter将总共贡献$d$的代价，因此从$T_1$出发的这部分Filter的代价为恒定的量$\vert T_1 \vert d$.
