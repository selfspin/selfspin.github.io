...
令$S_n$表示第$n$个顾客的到达时间，$X_n = S_n -S_{n-1}$表示经历的等待时间，由上定义了两个随机变量。
...
$$
f(s) = \lim \frac{(\lambda s)^{n-1} \exp(-\lambda s)}{ (n-1)!} (\lambda h +o(h)) = \frac{\lambda \exp(-\lambda s) (\lambda s)^{n-1} }{ (n-1)!}
$$
...
利用矩母函数，$M_Z(w) = E[\exp(w Z)] = E_N[\prod_{i=1}^N E[\exp(w X_i)]] = E_N[M_X(w)^N]$
...
可以证明，非齐次泊松过程满足，$ P(N(s,t)=n) = \frac{(m(s,t))^n \exp(-m(s,t))}{n!}$

首先通过微分方程的方式证明：$P_n(t) = P(N(t)=n) = \frac{m(t)^n \exp(-\lambda m(t))}{n!}$  
...
递推可以得到，$P(N(t)=n) = \frac{m(t)^n \exp(-\lambda m(t))}{n!}$

根据独立增量性，使用类似的方法可以得到：$ P(N(s,t)=n) = \frac{(m(s,t))^n \exp(-m(s,t))}{n!}$

对于非齐次泊松过程的等待时间，不再满足指数分布，且由于$m(s,t)$与起点和终点的位置相关，$X_i$之间不再独立。
