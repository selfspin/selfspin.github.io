...
沿用上一节的分析思路，令 $ x \in ({K_{\sigma,n})^m $ 满足 $x$ 为决策空间均为 $K_{\sigma,n}$ 的博弈的0-Nash均衡点，然后根据Lemma 3.2 我们可以选择 $k$ 然后对于每一个 $j \in [m]$ 都采样对应的 $B_{1,j}, \cdots B_{k,j} \sim x_j $ 然后计算其样本均值 $\hat X_j$ 就可以以概率 $1-\delta$得到一个-近似-$\sigma$-光滑-Nash均衡点。
