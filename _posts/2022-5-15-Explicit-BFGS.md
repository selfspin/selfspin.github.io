...
可以证明该迭代格式具有如下的性质：对于某一正数 $\eta$ , 若 $ A \preceq G_k \preceq \eta A$， 那么则有 $ A \preceq G_{k+1} \preceq \eta A$. 该性质的证明告诉我们，如果我们对想要近似的矩阵 $A$ 有一个比较好的初始估计范围的话，算法给出的矩阵迭代序列都将在这个范围内。实际上该性质对于整一类Broyden算法都是成立的，我将相关证明其放在如下链接中 [5]. 
